...
ClaimResult PgOfferingRepo::claim(const ClaimParams &params) {
    ...
    const auto offer_rows = txn->execSqlSync(
        "SELECT o.id, o.item_instance::text AS item_instance, "
    ...
    const auto pay_rows = txn->execSqlSync(
        "SELECT type_id, version "
        "FROM item_instance "
    ...
    const auto release_rows = txn->execSqlSync(
    ...
    const auto pay_update_rows = txn->execSqlSync(
        "UPDATE item_instance "
        "SET holder        = $1, "
        "    version       = version + 1, "
        "    custody_depth = custody_depth + 1 "
