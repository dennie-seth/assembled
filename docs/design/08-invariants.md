...
**INV-9 is the scaling law.** The *law* is population-independent; only its parameters are tuned. Concretely: a run is up to 18 rooms at P=2 and up to 18 rooms at P=10⁵, assembled from **exactly 3 archetypes** of varying authored size (5–8 rooms each); what grows is which archetypes and variants are eligible to appear, not the room count. If the two-player experience and the ten-thousand-player experience differ in kind rather than degree, INV-9 has been violated and the design has a scaling bug regardless of what the code does.
