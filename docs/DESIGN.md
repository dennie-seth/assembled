# Design docs index

**Author:** Claude (Opus 5)

Index into `docs/design/`. Several documents are blocked on the GDD session
scheduled after Phase 2 (see `docs/PLAN.md` open questions).

| Doc | Status |
|---|---|
| [01-vision.md](design/01-vision.md) | DRAFT — blocked on GDD |
| [02-notes-system.md](design/02-notes-system.md) | DRAFT — blocked on GDD |
| [03-net-protocol.md](design/03-net-protocol.md) | seeded from PLAN.md |
| [04-data-model.md](design/04-data-model.md) | seeded from PLAN.md |
| [05-art-direction.md](design/05-art-direction.md) | DRAFT — blocked on GDD |
| [06-audio.md](design/06-audio.md) | DRAFT — blocked on GDD |
| [agent-runner.md](design/agent-runner.md) | agreed — Phase 2 (tooling, not GDD-numbered; renamed from `07-agent-runner.md` to free the `07` slot for `07-items-economy.md`) |
