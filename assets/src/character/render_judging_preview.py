#!/usr/bin/env python3
"""Render Arm A's delivered sheet as a judgeable, in-motion preview (T-0228).

DL-21 (`docs/decision-log.md`) judges every bake-off arm "at 40px, in motion,
inside the T-0192 blockout room" -- explicitly not at native generation
resolution and not as a static contact sheet, since both flatter detail the
descent destroys. This repo has no committed screenshot of the blockout room
(`client/scenes/blockout_room_sideon.tscn` is a Godot scene; rendering it is
outside this agent's tool scope, see `.claude/agents/assets.md`), so this
composites onto a flat mockup at the room's own committed pixel dimensions
(384x216, DL-18) instead of an actual in-engine capture -- a size/motion
preview, not a claim of an in-engine screenshot.

Usage (from the repo root):
    python3 assets/src/character/render_judging_preview.py

Reads:
    assets/final/character/player_idle_sheet_arm_a_T0228.png
Writes:
    assets/final/character/arm_a_judging_preview_T0228.gif
"""
...
OUT_PATH = REPO_ROOT / "assets" / "final" / "character" / "arm_a_judging_preview_T0228.gif"
