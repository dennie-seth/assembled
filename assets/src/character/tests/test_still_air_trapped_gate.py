"""Still Air entity trapped sheet — T-0200 gate validation.

docs/design/13-asset-pipeline.md §3.5 (Characters — the hard class):
  cell 48×48, grid 2×2, native 96×96, entity figure ~32px tall (narrow column).

Sheet: assets/final/entity/still_air_trapped_sheet_v1.png (4 trapped/delayed frames).

Still Air: atmospheric column entity — constrained/delayed pose (no die state).

RED state:  assets/final/entity/still_air_trapped_sheet_v1.png absent
            → SHEET_PATH fixture raises AssertionError, all tests ERROR.
GREEN state: sheet present, mode P, 96×96; passes palette-membership,
             index-semantics, cell-fit (2×2, 48×48), orphan-pixel per frame
             cell, and frame-consistency between adjacent trapped frames.
...
SHEET_PATH = REPO_ROOT / "assets" / "final" / "entity" / "still_air_trapped_sheet_v1.png"
...
def sheet() -> Image.Image:
    assert SHEET_PATH.exists(), (
        f"still_air trapped sheet not found: {SHEET_PATH}\n"
        "Run `generate-synth-still-air-trapped-sheet` to produce it."
    )
