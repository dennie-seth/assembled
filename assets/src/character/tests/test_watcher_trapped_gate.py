"""Watcher entity trapped sheet — T-0200 gate validation.

docs/design/13-asset-pipeline.md §3.5 (Characters — the hard class):
  cell 48×48, grid 2×2, native 96×96, entity figure ~20px tall.

Sheet: assets/final/entity/watcher_trapped_sheet_v1.png (4 trapped/delayed frames).

Watcher: surveillance orb entity — constrained/delayed pose (no die state).

RED state:  assets/final/entity/watcher_trapped_sheet_v1.png absent
...
SHEET_PATH = REPO_ROOT / "assets" / "final" / "entity" / "watcher_trapped_sheet_v1.png"
...
def sheet() -> Image.Image:
    assert SHEET_PATH.exists(), (
        f"watcher trapped sheet not found: {SHEET_PATH}\n"
        "Run `generate-synth-watcher-trapped-sheet` to produce it."
    )
