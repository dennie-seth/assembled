"""Still Air entity move sheet — T-0200 gate validation.

docs/design/13-asset-pipeline.md §3.5 (Characters — the hard class):
  cell 48×48, grid 4×2, native 192×96, entity figure ~32px tall (narrow column).

Sheet: assets/final/entity/still_air_move_sheet_v1.png (8 move frames).

Still Air: atmospheric column entity — horizontal drift/sway animation.

RED state:  assets/final/entity/still_air_move_sheet_v1.png absent
            → SHEET_PATH fixture raises AssertionError, all tests ERROR.
GREEN state: sheet present, mode P, 192×96; passes palette-membership,
             index-semantics, cell-fit (4×2, 48×48), orphan-pixel per frame
             cell, and frame-consistency between adjacent move frames.
...
SHEET_PATH = REPO_ROOT / "assets" / "final" / "entity" / "still_air_move_sheet_v1.png"
...
def sheet() -> Image.Image:
    assert SHEET_PATH.exists(), (
        f"still_air move sheet not found: {SHEET_PATH}\n"
        "Run `generate-synth-still-air-move-sheet` to produce it."
    )
...
def test_frame_consistency(
    ...
    """Silhouette delta between adjacent move frames must stay within MAX_FRAME_DELTA_RATIO."""
