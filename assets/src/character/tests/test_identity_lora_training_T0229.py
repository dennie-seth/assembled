"""Player-identity LoRA training gate — T-0229 (HANDOFF §23-e, DL-21).

Acceptance: "The trained LoRA is itself covered by provenance (weights hash +
training config + curation set resolvable), not just the generation step."
This gate validates the trained weights file and its provenance sidecar
independently of the idle-sheet generation gate (test_player_idle_arm_b_gate.py)
-- training is its own deliverable, not merely an ingredient.

RED state: assets/final/lora/player_identity_v1.safetensors absent -> every
...
def provenance() -> dict:
    assert PROVENANCE_PATH.exists(), (
        f"identity LoRA provenance sidecar not found: {PROVENANCE_PATH}\n"
        "Run assets/src/lora/src/lora_train/train.py against "
        "training_config_player_identity_T0229.toml to produce the weights, then "
        "write the sidecar (see gen_arm_b_idle_T0229.py / the curation+train workflow)."
    )
    return json.loads(PROVENANCE_PATH.read_text())


def test_weights_file_is_valid_safetensors() -> None:
    assert WEIGHTS_PATH.exists(), f"trained identity LoRA not found: {WEIGHTS_PATH}"
...
def test_training_cost_fields_present(provenance: dict) -> None:
    """Acceptance: curation + training time are real costs, counted in this
    arm's cost row -- the provenance sidecar is where that number is anchored."""
