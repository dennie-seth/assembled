"""Sound entity idle sheet v2 — T-0214 gate validation.

Concept-conditioned re-run of T-0200 sound idle sheet, conditioned on the
...
docs/design/13-asset-pipeline.md §3.5 (Characters — the hard class):
  cell 48×48, grid 3×2, native 144×96, entity figure ~12px tall.

Sheet: assets/final/entity/sound_idle_sheet_v2.png (6 idle frames).

Sound: wide flat wave-band entity — ambient and expansive. No die state.

RED state:  sound_idle_sheet_v2.png absent → fixture raises AssertionError.
GREEN state: sheet present, mode P, 144×96; passes all gate checks,
...
SHEET_PATH = REPO_ROOT / "assets" / "final" / "entity" / "sound_idle_sheet_v2.png"
PROV_PATH = REPO_ROOT / "assets" / "final" / "entity" / "sound_idle_sheet_v2.provenance.json"
...
def sheet() -> Image.Image:
    assert SHEET_PATH.exists(), (
        f"sound idle sheet v2 not found: {SHEET_PATH}\n"
