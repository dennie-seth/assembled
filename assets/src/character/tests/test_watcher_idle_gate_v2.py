"""Watcher entity idle sheet v2 — T-0214 gate validation.

Concept-conditioned re-run of T-0200 watcher idle sheet, conditioned on the
...
docs/design/13-asset-pipeline.md §3.5 (Characters — the hard class):
  cell 48×48, grid 3×2, native 144×96, entity figure ~20px tall.

Sheet: assets/final/entity/watcher_idle_sheet_v2.png (6 idle frames).

Watcher: surveillance orb entity — wide, compact, floating. No die state.

RED state:  assets/final/entity/watcher_idle_sheet_v2.png absent
            → SHEET_PATH fixture raises AssertionError, all tests ERROR.
GREEN state: sheet present, mode P, 144×96; passes palette-membership,
             index-semantics, cell-fit (3×2, 48×48), orphan-pixel per frame
             cell, frame-consistency between adjacent idle frames, and
             concept_hash present in provenance (T-0106).
...
SHEET_PATH = REPO_ROOT / "assets" / "final" / "entity" / "watcher_idle_sheet_v2.png"
PROV_PATH = REPO_ROOT / "assets" / "final" / "entity" / "watcher_idle_sheet_v2.provenance.json"
...
def sheet() -> Image.Image:
    assert SHEET_PATH.exists(), (
        f"watcher idle sheet v2 not found: {SHEET_PATH}\n"
...
def test_frame_consistency(
    ...
    """Silhouette delta between adjacent idle frames must stay within MAX_FRAME_DELTA_RATIO."""
