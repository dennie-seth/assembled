"""Player die sheet v2 — T-0213 gate validation (concept-conditioned).
...
GREEN state: sheet present, mode P, 144×144; passes palette-membership,
             ...
             cell, frame-consistency between adjacent die frames, and
...
SHEET_PATH = REPO_ROOT / "assets" / "final" / "character" / "player_die_sheet_v2.png"
PROVENANCE_PATH = (
...
# Adjacent pairs for frame-consistency (death sequence order)
...
BACKGROUND_INDEX = 0
# Death sequence has the most radical silhouette change (vertical → horizontal) — 90% max delta.
MAX_FRAME_DELTA_RATIO = 0.90
...
def sheet() -> Image.Image:
    assert SHEET_PATH.exists(), (
        f"v2 die sheet not found: {SHEET_PATH}\n"
        "Run gen_die_v2_tiled.py to produce it via ComfyUI img2img."
    )
...
def test_concept_hash() -> None:
    ...
    assert PROVENANCE_PATH.exists(), (
        ...
        "Commit player_die_sheet_v2.provenance.json with a concept_hash field."
    )
...
def test_frame_consistency(
    ...
    """Silhouette delta between adjacent die frames must stay within MAX_FRAME_DELTA_RATIO."""
