"""Player crouch-hide sheet — T-0199 gate validation.
...
Sheet: player_crouch_hide_sheet_v1.png (9 crouch-hide frames, 0 spare cells).

Crouch animation: figure progressively compresses from standing height (~40px)
to a crouched position (~25px) then holds with minor breathing variation.

RED state:  assets/final/character/player_crouch_hide_sheet_v1.png absent
...
GREEN state: sheet present, mode P, 144×144; passes palette-membership,
             ...
             cell, and frame-consistency between adjacent crouch frames.
...
SHEET_PATH = REPO_ROOT / "assets" / "final" / "character" / "player_crouch_hide_sheet_v1.png"
...
BACKGROUND_INDEX = 0
# Crouch transitions can have larger silhouette change as figure compresses — 65% max delta.
...
def sheet() -> Image.Image:
    assert SHEET_PATH.exists(), (
        f"crouch-hide sheet not found: {SHEET_PATH}\n"
        "Run `generate-synth-crouch-hide-sheet` to produce it."
    )
