"""Player-identity PROFILE-pose LoRA training gate — T-0274.

Same shape as test_identity_lora_training_T0248.py (v2's gate, left intact):
validates the trained weights file and its provenance sidecar independently
of any downstream generation gate. Additionally checks that this is a
strictly additive artifact -- `player_identity_v2` is untouched.

RED state: assets/final/lora/player_identity_profile_v1.safetensors absent
...
WEIGHTS_PATH = REPO_ROOT / "assets" / "final" / "lora" / "player_identity_profile_v1.safetensors"
PROVENANCE_PATH = (
    REPO_ROOT / "assets" / "final" / "lora" / "player_identity_profile_v1.provenance.json"
)

V2_WEIGHTS_PATH = REPO_ROOT / "assets" / "final" / "lora" / "player_identity_v2.safetensors"
V2_PROVENANCE_PATH = REPO_ROOT / "assets" / "final" / "lora" / "player_identity_v2.provenance.json"
...
def provenance() -> dict:
    assert PROVENANCE_PATH.exists(), (
        f"profile identity LoRA provenance sidecar not found: {PROVENANCE_PATH}\n"
        "Run assets/src/lora/src/lora_train/train.py against "
        "training_config_player_identity_profile_v1.toml to produce the weights, "
...
def test_weights_file_is_valid_safetensors() -> None:
    assert WEIGHTS_PATH.exists(), f"trained profile identity LoRA not found: {WEIGHTS_PATH}"
...
def test_training_cost_fields_present(provenance: dict) -> None:
    """Acceptance: hyperparameters, steps, and actual GPU cost are recorded
    to the same standard player_identity_v2's provenance record meets."""
...
def test_smoke_check_field_present(provenance: dict) -> None:
    """Acceptance: a smoke check demonstrating the LoRA actually places a
    profile through the §24-e stack -- recorded so the claim is checkable,
    not just asserted in a commit message."""
    smoke_check = provenance.get("smoke_check")
    assert isinstance(smoke_check, dict), "provenance missing 'smoke_check' object"
    assert "result" in smoke_check, "smoke_check missing 'result'"
    assert smoke_check.get("evidence"), "smoke_check missing 'evidence' (report path)"


def test_v2_weights_and_provenance_left_intact() -> None:
    """Acceptance: player_identity_v2 is untouched and unregressed -- this
    profile LoRA is a separate, additive artifact."""
    assert V2_WEIGHTS_PATH.exists(), f"v2 weights missing: {V2_WEIGHTS_PATH}"
    assert V2_PROVENANCE_PATH.exists(), f"v2 provenance missing: {V2_PROVENANCE_PATH}"
