"""Player move (walk) sheet v2 — T-0213 gate validation (concept-conditioned).

docs/design/13-asset-pipeline.md §3.5 (Characters — the hard class):
  cell 48×48, grid 3×4, native 144×192, figure 40px tall.

Differences from T-0199 (test_player_move_gate.py / v1):
  - Targets player_move_sheet_v2.png — concept-conditioned, img2img-seeded
...
RED state:  assets/final/character/player_move_sheet_v2.png absent
...
GREEN state: sheet present, mode P, 144×192; passes palette-membership,
             ...
             cell, frame-consistency between adjacent walk frames, and
...
SHEET_PATH = REPO_ROOT / "assets" / "final" / "character" / "player_move_sheet_v2.png"
PROVENANCE_PATH = (
...
# This is the img2img conditioning seed for the T-0213 state-sheet generation.
...
def sheet() -> Image.Image:
    assert SHEET_PATH.exists(), (
        f"v2 move sheet not found: {SHEET_PATH}\n"
        "Run gen_move_v2_tiled.py to produce it via ComfyUI img2img."
    )
...
def test_concept_hash() -> None:
    ...
    assert PROVENANCE_PATH.exists(), (
        ...
        "Commit player_move_sheet_v2.provenance.json with a concept_hash field."
    )
