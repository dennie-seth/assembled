...
| 8 | 31416 | 1.0/1.0 | 0.7 | 0.5 | 0.05 | 0.6 | 42.1 | PASS | no | final attempt, DL-21 cap: same seed as 1/5/7, pose LoRA weight reduced to near-zero (0.05) to isolate whether attempts 5/7's wrong-subject failure is weight-driven or structural (an extra chained LoRA node in the graph, not its learned weights). Visual verdict: still the same monitor/bottle wrong-subject shape as attempts 5 and 7 -- did NOT recover attempt 1's original boxy-but-human result, even at a weight low enough that the LoRA's own learned delta should be negligible. |
...
The result is not an improvement -- it is qualitatively worse. **Zero of the
four stacked attempts produced even a human silhouette**, wrong-facing or
otherwise; all four collapsed into a "wrong subject" failure (T-0218's failure
class), reading as inanimate objects (a monitor/bottle at seed 31416 across
attempts 5, 7, and 8; a control panel with LED readouts at seed 27182 in
attempt 6, matching attempt 3's failure at that same seed). By contrast, round
1's attempts 1 and 4 (same seed 31416, no pose LoRA) at least produced a
human-shaped, if wrong-facing, figure.

**Attempt 8's near-zero pose-LoRA weight (0.05) is the important new data
point.** If the pose LoRA's own learned weights were driving the collapse, a
weight low enough to make its contribution negligible should have recovered
something close to attempt 1's original boxy-but-human result. It did not --
attempt 8 is visually near-identical to attempts 5 and 7. That rules out
"the pose LoRA's training content is wrong" as the explanation and points
instead at something structural: simply routing the CLIP text encoders and the
IP-Adapter's MODEL input through an additional chained `LoraLoader` node (even
one applying a near-zero delta) changes the sampling outcome on this
checkpoint/seed combination. Whether that is a ComfyUI-side interaction
(node-graph shape affecting some cached or precision-sensitive computation) or
an SDXL-side sensitivity to an extra LoRA hop in the CLIP conditioning path is
not established by this card -- only that the effect is real and reproducible
across three separate weight settings (0.6, 0.3, 0.05) at the same seed.
...
1. The stacked-LoRA structural sensitivity attempt 8 surfaced is worth its own
   investigation before spending more generation budget on this pose --
   e.g. whether merging the pose LoRA's weights into a single LoRA file
   offline (rather than chaining a second `LoraLoader` node at generation
   time) avoids whatever this graph shape is doing to subject coherence.
