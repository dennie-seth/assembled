#!/usr/bin/env python3
"""Render the pose-authority sheet as a judgeable, in-motion preview (T-0249).

Mirrors render_judging_preview_arm_c_T0230.py / render_judging_preview_arm_b_T0229.py
-- round-2 rules (HANDOFF §24.3) keep DL-21's judging conditions unchanged: judged
"at 40px, in motion, inside the T-0192 blockout room", not at native generation
resolution and not as a static contact sheet. This repo has no committed screenshot
of the blockout room (`client/scenes/blockout_room_sideon.tscn` is a Godot scene,
outside this agent's tool scope), so this composites onto the same flat mockup at
the room's own committed pixel dimensions (384x216, DL-18) the other two arms use.

Usage (from the repo root):
    python3 assets/src/character/render_judging_preview_pose_authority_T0249.py

Reads:
    assets/final/character/player_idle_sheet_pose_authority_T0249.png
Writes:
    assets/final/character/pose_authority_judging_preview_T0249.gif
"""
...
OUT_PATH = (
    REPO_ROOT / "assets" / "final" / "character" / "pose_authority_judging_preview_T0249.gif"
)
