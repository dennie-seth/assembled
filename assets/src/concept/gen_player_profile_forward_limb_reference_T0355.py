...
POSITIVE_PROMPT = (
    "single full-colour game character action illustration, exactly one "
    "isolated figure, flat side-on orthographic silhouette study, absolutely "
    "no perspective, no vanishing point, character caught mid-motion in a "
    "walking action pose: figure's face and nose seen in strict profile "
    "facing right, one shoulder visible, (near arm bent forward at the "
    "elbow to roughly 90 degrees, reaching out ahead of the torso:1.4), "
    "(near leg lifted and swung forward at the hip to roughly 90 degrees, "
    "knee bent, foot lifted clear of the ground:1.4), far arm and far leg "
    "hidden behind the body, only one arm silhouette visible in total, only "
    "one hand visible in total, weight on the trailing leg, dynamic mid-step "
    "silhouette caught mid-motion, absolutely not a static standing pose, "
    "absolutely not arms hanging at the sides. Player character: 40px tall "
    ...
    "the knee:1.3), coat fabric swinging with the stride's own motion, "
    "(hooded mask with two dark round visible eye lenses, not a blank void, "
    "hood fully up and forward, face completely covered by the mask, no "
    "visible hair, no visible face:1.3), only one eye lens visible in strict "
    "profile, white gloves on both hands, dark boots on both feet, "
    "concrete-grey exposed skin tones, deep shadow values on the coat's own "
    "folds. Soviet brutalist interior aesthetic, muted desaturated palette "
    "for the environment only, the coat itself stays vivid green, hard value "
    "separation, dark darks and light lights, flat even lighting, no "
    "atmospheric haze, no depth of field, no background elements, solid flat "
    "black background only, pixel art scale reference, figure silhouette "
    "study"
)
...
NEGATIVE_PROMPT = (
    ...
    "people, "
    "reference sheet, concept sheet, character sheet, model sheet, "
    "turnaround, character turnaround, multiple panels, multiple views, "
    "side-by-side panels, comparison chart, thumbnail row, colour swatches, "
    "palette swatches, colour palette chips, line art sketches, sketch "
    "lineup, multiple poses, pose variations grid, orthographic sheet, "
    "bare head, bald head, visible hair, visible human face, exposed face, "
    "no mask, unmasked, bare hands, bare feet, barefoot, both eye lenses "
    "visible, two eye lenses visible, front of mask visible, two arms "
    "visible, two hands visible, both arms extended, "
