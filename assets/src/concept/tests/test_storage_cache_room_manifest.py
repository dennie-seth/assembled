"""T-0241 — Signal Tower / Storage Cache room prop manifest gate (HANDOFF §23-j-e).

Storage Cache (`signal_tower.storage_cache`) is a small, cramped closet needing
exactly two prop slots per `docs/design/14-vertical-slice.md` §10: stored crates
(primary dressing) and a closet locker. Both slots are fully covered by props
already committed under `assets/final/props/signal_tower/` (T-0201/T-0221/
T-0223) against the approved `signal_tower_props_concept_sheet_v1.png` sheet
(DL-5) — this card generates no new prop geometry, it only records the room's
slot -> prop mapping and re-confirms the 16px cover/hiding separation gate for
the specific pair this room places together.

This is the same room pair as Ground Relay (T-0240): crate_stack_v1 (cover)
and locker_v1 (hiding) — so the measured gap is identical, re-derived
independently here rather than assumed from the sibling card.
...
Gate references:
  ...
  - P-3 — 16px legibility: >= 15.0 luma16 gap between any cover/hiding pair
    ...
    this file re-derives the gap for Storage Cache's actual pair
    (crate_stack_v1 cover vs locker_v1 hiding) and checks it against the
  ...
  - P-7 (amended 2026-08-29) — provenance is required only for props a card
    *newly generates*. Storage Cache reuses two already-committed,
...
MANIFEST_PATH = PROPS_DIR / "storage_cache.manifest.json"
...
def test_manifest_identifies_room(manifest):
    assert manifest["room"] == "signal_tower.storage_cache"
...
def test_manifest_has_exactly_the_two_slots(manifest):
    ...
    assert slot_names == {
        "Stored crates (primary dressing)",
        "Closet locker",
    }
...
def test_manifest_slot_classes_match_expected_props(manifest):
    expected = {
        "Stored crates (primary dressing)": ("cover", "crate_stack_v1"),
        "Closet locker": ("hiding", "locker_v1"),
    }
    for slot in manifest["slots"]:
...
def test_manifest_gate_measurement_clears_floor(manifest):
    """Storage Cache's actual cover/hiding pair must clear the P-3 floor."""
    gap = manifest["gate_measurements"]["gap"]
    assert gap >= _MIN_COVER_HIDE_GAP, (
        f"Storage Cache pair gap {gap:.1f} is below the required "
