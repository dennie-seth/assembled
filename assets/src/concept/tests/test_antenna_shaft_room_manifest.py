"""T-0246 -- Signal Tower / Antenna Shaft room prop manifest gate (HANDOFF §23-j-f).

Antenna Shaft (`signal_tower.antenna_shaft`) needs two prop slots per this
card's own slot table (mirroring `docs/design/14-vertical-slice.md` §10's row
for this room: "Narrow vertical shaft, winding ladder path. The Still Air on
a fixed lap, no LOS -- avoidance is pure timing, not routing. At least one
hiding alcove partway up -- the room's only reliable safety valve."):

**Shaft-wall duct dressing slot -- resolved to `low_duct_v1` (reused, cover
class, already committed from T-0201/T-0221/T-0223).** This card's own slot
table marks it "committed" -- no ambiguity to resolve, unlike Equipment
Floor's server_rack question.

**Hiding alcove slot -- resolved to `hiding_alcove_v1` (new, hiding class).**
§10 calls this room's alcove "the room's only reliable safety valve" against
The Still Air, which runs a fixed patrol lap with **no LOS check** -- cover is
useless here by design, so a locker (free-standing furniture) cannot
substitute for an alcove set into the shaft wall. The v1 sheet has no
coverage for this at all. `signal_tower_props_concept_sheet_v3.png` (T-0257,
APPROVED 2026-08-30, PR #307) depicts a dedicated "HIDING ALCOVE -- HIDING
SPOT" sub-panel (`panels[3]` in the recipe, seed 23121, distinct from the
sibling "CRAWLSPACE OPENING" sub-panel T-0245 already used for Equipment
Floor) -- DL-5 concept coverage for exactly this slot. Generated through the
committed cutout path (`tools/comfy-client/src/comfy_client/cutout.py`),
reusing that sub-panel's own reviewed prompt text verbatim (only cutout
dimensions/seed differ, same pattern as T-0244/T-0245).

Gate references:
  - DL-5 / P-6 -- concept art precedes generation. Verified here by resolving
    hiding_alcove_v1's `concept_hash` against the approved v3 sheet's own
    sha256.
  - P-3 -- 16px legibility: >= 15.0 luma16 gap between any cover/hiding pair
    co-located in this room. One pair: low_duct_v1/hiding_alcove_v1.
  - P-7 (amended 2026-08-29) -- provenance `concept_hash`/`generator`
    resolvability is required only for hiding_alcove_v1, the prop this card
    newly generates. low_duct_v1 is reused as-is, already provenanced by
    T-0201/T-0221/T-0223.
"""
...
MANIFEST_PATH = PROPS_DIR / "antenna_shaft.manifest.json"
...
NEW_PROPS = ["hiding_alcove_v1"]
COVER_PROPS = ["low_duct_v1"]
HIDING_PROP = "hiding_alcove_v1"
...
def test_manifest_identifies_room(manifest):
    assert manifest["room"] == "signal_tower.antenna_shaft"


def test_manifest_declares_new_geometry(manifest):
    """This card DOES generate new props (the hiding alcove)."""
...
def test_manifest_has_exactly_the_two_slots(manifest):
    ...
    assert slot_names == {
        "Hiding alcove (partway up)",
        "Shaft-wall duct dressing",
    }
...
def test_t0257_concept_sheet_is_approved():
    ...
    assert "APPROVED" in row, (
        ...
        "slot (the hiding alcove) may be generated (DL-5)."
    )


# ── Slot -> prop resolution ──────────────────────────────────────────────────

def test_shaft_wall_duct_slot_resolves_to_reused_cover_prop(manifest):
    slot = _slot(manifest, "Shaft-wall duct dressing")
    assert slot["class"] == "cover"
    assert slot["status"] == "committed"
    assert slot["reused"] is True
    assert slot["prop"] == "low_duct_v1"
    assert (PROPS_DIR / "low_duct_v1.png").exists()
    assert (PROPS_DIR / "low_duct_v1.provenance.json").exists()


def test_hiding_alcove_slot_resolves_to_one_new_hiding_prop(manifest):
    slot = _slot(manifest, "Hiding alcove (partway up)")
    assert slot["class"] == "hiding"
    assert slot["status"] == "committed"
    assert slot["reused"] is False
    assert slot["prop"] == "hiding_alcove_v1"
    assert (PROPS_DIR / "hiding_alcove_v1.png").exists()
    assert (PROPS_DIR / "hiding_alcove_v1.provenance.json").exists()
...
def test_gate_measurements_present_for_the_cover_hiding_pair(manifest):
