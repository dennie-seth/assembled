#!/usr/bin/env python3
"""Forward-limb green side reference via ControlNet/OpenPose img2img (T-0380).

T-0355's stop-and-report (`ARM_FORWARD_LIMB_REFERENCE_ATTEMPT_LOG_T0355.md`)
falsified the prompt-only route for this exact pose (near arm + near leg
extended ~90 degrees, strict profile, canonical hooded-mask/glove/boot
identity): attempt 4 got costume and identity right, but the upper body
...
no ControlNet, no img2img):
  ...
  - **OpenPose ControlNet** on the authored forward-limb skeleton
    ...
    committed T-0351 rig's `side_right_forward`) supplies the pose
    structurally, the same lever T-0351's own evidence shows dominates a
    text prompt's camera-angle/pose request.
...
Usage (from the repo root, against the WSL2->Windows ComfyUI host):
    python3 assets/src/concept/gen_player_profile_forward_limb_controlnet_T0380.py \\
        --attempt 1 --seed 380001 --denoise 0.78 --controlnet-strength 1.4
    python3 assets/src/concept/gen_player_profile_forward_limb_controlnet_T0380.py \\
        --promote-attempt 1

Writes (always, so every attempt is logged whether it is promoted or not):
    assets/out/forward_limb_controlnet_T0380/attempt_<N>/main_1024.png
    assets/out/forward_limb_controlnet_T0380/attempt_<N>/provenance_candidate.json
    assets/src/concept/ARM_FORWARD_LIMB_REFERENCE_CONTROLNET_ATTEMPT_LOG_T0380.md (appended)
...
DEFAULT_CONTROLNET_STRENGTH = 1.4
...
POSITIVE_PROMPT = (
    ...
    "silhouette, a single visible eye lens, both hands empty, open palm, nothing held, wearing "
    "a long vivid institutional green cloth coat, full-length coat reaching past the knee, the "
    "coat itself is green, not black, not grey, wearing a hooded mask with a single dark round "
    "visible eye lens, not a blank void, hood fully up and forward, face completely covered by "
    "the mask, no visible hair, no visible face, white gloves on both hands, boots, never high "
    "heels, Soviet brutalist interior aesthetic, solid flat black background only, no scene "
    "elements, no wall, no floor, no environment, flat even lighting, no cast shadow, no "
    "atmospheric haze, no depth of field, hard value separation, dark darks and light lights, "
    "clean readable outline, game asset reference sheet style, figure silhouette study"
)
...
def build_graph(
    ...
    g[SAVE_NODE_ID] = {
        ...
        "inputs": {
            "filename_prefix": "forward_limb_controlnet_T0380_main_1024",
...
ATTEMPT_LOG_PATH = (
    ...
    / "src"
    / "concept"
    / "ARM_FORWARD_LIMB_REFERENCE_CONTROLNET_ATTEMPT_LOG_T0380.md"
)
FINAL_PATH = (
...
ATTEMPT_LOG_HEADER = (
    ...
    "Every attempt is recorded here whether promoted or not. Pose is supplied by an OpenPose "
    "ControlNet skeleton (`pose_rig_forward_limb_controlnet_T0380`, reused from the committed "
    "T-0351 `side_right_forward` rig), not the prompt -- see the generator module's own "
    "docstring for why (T-0355's prompt-only route was falsified). img2img from the T-0317 "
    "base, denoise/ControlNet strength tuned per attempt within this card's own 4-attempt cap.\n\n"
    "| Attempt | Seed | Denoise | ControlNet strength/end | GPU seconds | Whole-frame green px | "
    "Centred-crop green px | Border max channel | Promoted | Notes |\n"
    "|---|---|---|---|---|---|---|---|---|---|\n"
)
...
def run_attempt(
    ...
    out_dir = REPO_ROOT / "assets" / "out" / "forward_limb_controlnet_T0380" / f"attempt_{attempt}"
    ...
    provenance = {
        ...
        "method": (
            ...
            "rig) + LoraLoader(style) -> LoraLoader(identity), chained. No IP-Adapter."
        ),
        "generator": "assets/src/concept/gen_player_profile_forward_limb_controlnet_T0380.py",
        "card": "T-0380",
        "route": "generated",
        "spec": (
            "docs/design/13-asset-pipeline.md, ARM_FORWARD_LIMB_REFERENCE_ATTEMPT_LOG_T0355.md's "
            "own stop-and-report finding (pose-by-prompt falsified)"
        ),
        "attempt": attempt,
...
def main() -> None:
    ...
    if not (1 <= args.attempt <= 4):
        raise SystemExit("attempt cap is 4 (this card's own pre-registered hard cap)")
    ...
    out_dir = REPO_ROOT / "assets" / "out" / "forward_limb_controlnet_T0380" / f"attempt_{args.attempt}"
    (out_dir / "provenance_candidate.json").write_text(json.dumps(provenance, indent=2) + "\n")
