...
2. Run the matching command(s):
   ...
   | `client/**` (`.gd`/`.tscn`) | `godot --headless` gdUnit4 test run. For any changed `client/tests/*.gd` file specifically, this is code-enforced via `verifyRouter.js`'s `client-godot-verify` route: `cd client && timeout 600 godot --headless --script tests/<file>.gd` — **always timeout-wrapped, never bare**. A test that never calls `get_tree().quit()` hangs indefinitely otherwise (T-0185); `timeout` killing the run is itself a FAIL, not an unverified pass. |
