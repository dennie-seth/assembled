 ...
 * T-0317 rounds 10-12 found forcing that flag (and/or CUBLAS_WORKSPACE_CONFIG) reproducibly
 * destroys coherence on at least one real graph, so baseline is sometimes the correct, deliberate
 * live state -- this check exists to catch an ACCIDENTAL drift in either direction (a bare
 * relaunch reverting to baseline when determinism was wanted, or someone leaving --deterministic
 * on for a graph that needs baseline), not to enforce one fixed opinion.
 *
