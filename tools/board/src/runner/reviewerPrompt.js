...
function buildRequiredVerificationSection(changedPaths, baseBranch, task) {
  ...
  if (hasDeliverableRoute) {
    ...
    enforcement += ` **This gate is necessary but not sufficient -- a zero exit is not proof a real deliverable was produced.** T-0351 saw \`checkDeliverable.js\` exit 0 twice in one day on the same six failed-attempt evidence PNGs -- attachments recorded and present on disk, but no master sheet ever committed -- and it took a human reading the filesystem by hand to catch it both times. A card can name its expected deliverable's committed path under a \`"${DELIVERABLE_HEADING}"\` section (a backtick-quoted path, same citation shape as \`"${FINDING_HEADING}"\`'s evidence citations) and the check will confirm that path is a real file under the repo; when a card has no such section, the check falls back to requiring at least one recorded attachment's *content* to byte-match a file \`git\` actually has committed on the branch, at a path outside \`docs/assets/evidence/**\` -- so attachment presence alone (an evidence/failed-attempt upload included) is no longer sufficient either way, and neither is a same-named-but-different file, nor evidence that only ever landed under the evidence root via T-0314's own promotion mechanism (which is the routine, encouraged path for evidence and was the actual second false positive here -- the fallback used to count a promoted evidence commit as proof of a deliverable). Even with that exclusion the fallback is still coarser than the opt-in route -- it only proves *something* was committed outside the evidence root, not that the specific claimed deliverable exists.`;
    enforcement += ` **T-0354 closed the third gap the same card exposed: existence without freshness.** A third consecutive T-0351 run committed only \`.py\` generator/test changes -- tests green, two commits nine seconds apart -- and never invoked the model at all; \`checkDeliverable.js\` still exited 0 because a master sheet promoted by an *earlier* run already satisfied existence. The check now also asserts freshness: the verified deliverable path (declared or fallback-matched) must have been touched by a commit made since this run's own start, not merely committed at some earlier point -- a stale, previously-promoted artifact can no longer rescue a run that generated nothing new, and this is asserted mechanically, not by hand-inspecting file timestamps across the repo. A freshness FAIL is distinct from "never tried" in two cases you should recognize in the script's own output rather than treat as a bare failure: a genuine host-only outage recorded as a \`\`\`host-action-request\`\`\` block (see this file's own footer and \`.claude/rules/conduct.md\`) in a commit made this run, and a pre-registered, decisive, evidenced finding (T-0342, below) -- both still PASS with no fresh artifact. Do not stop at the script's exit code alone, though: confirm what it actually reports as the verified/fresh path, and spot-check that it is the real deliverable (\`assets/final/**\` or wherever this card's deliverable belongs) and not merely a copy sitting under \`docs/assets/evidence/**\` -- but the mechanical freshness assertion, not a hand-driven repo sweep, is now the first line of defense for this failure mode.`;
