...
function buildRequiredVerificationSection(changedPaths, baseBranch, task) {
  ...
  if (hasDeliverableRoute) {
    ...
    enforcement += ` **This gate is necessary but not sufficient -- a zero exit is not proof a real deliverable was produced.** T-0351 saw \`checkDeliverable.js\` exit 0 twice in one day on the same six failed-attempt evidence PNGs -- attachments recorded and present on disk, but no master sheet ever committed -- and it took a human reading the filesystem by hand to catch it both times. A card can name its expected deliverable's committed path under a \`"${DELIVERABLE_HEADING}"\` section (a backtick-quoted path, same citation shape as \`"${FINDING_HEADING}"\`'s evidence citations) and the check will confirm that path is a real file under the repo; when a card has no such section, the check falls back to requiring at least one recorded attachment's filename to also correspond to a file \`git\` actually has committed on the branch -- so attachment presence alone (an evidence/failed-attempt upload included) is no longer sufficient either way. That fallback matches by filename only, not by declared path, so it is coarser than the opt-in route -- do not stop at the script's exit code: independently confirm the deliverable itself -- \`ls\`/read the actual committed file the card claims to have produced (\`assets/final/**\` or wherever this card's deliverable belongs) -- the same way the T-0351 reviewer ultimately had to.`;
