...
export function checkEdgeCasesPreflight(task) {
  ...
  return {
    warnings: [
      "## Acceptance has no **Edge cases:** block (.claude/rules/planner.md) -- the first " +
        "implementation pass has nothing forcing it to consider boundary/failure conditions " +
        "upfront; expect the reviewer to surface them one at a time across rework rounds instead."
    ]
  };
}
