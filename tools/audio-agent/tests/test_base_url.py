...
changes across WSL restarts (docs/comfyui-setup.md). ACESTEP_BASE_URL wins
when set; otherwise the Windows host IP comes from the live default route.
Mirrors tools/comfy-client/tests/test_base_url.py with ACESTEP_* env vars."""
...
from audio_agent.base_url import (
    DEFAULT_PORT,
...
def test_resolve_base_url_env_override_wins_and_strips_trailing_slash():
    env = {"ACESTEP_BASE_URL": "http://example.test:9000/"}
...
def test_resolve_base_url_honours_acestep_port_override():
    url = resolve_base_url(env={"ACESTEP_PORT": "9001"}, run=fake_run(IP_ROUTE_OUTPUT))
    assert url == "http://172.18.192.1:9001"
