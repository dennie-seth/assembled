"""ACE-Step HTTP client: POST /generate (blocking) -> GET /output/{filename}.
All calls are mocked via `responses` -- no live ACE-Step call in the suite."""
...
BASE_URL = "http://172.18.192.1:8001"


def make_client() -> AudioClient:
    return AudioClient(base_url=BASE_URL)


def test_submit_returns_output_path_as_job_id(sample_request):
    client = make_client()
    job_id = client.submit(sample_request)
    assert job_id == sample_request["output_path"]
...
def test_submit_raises_on_duplicate_job_id(sample_request):
    client = make_client()
...
@responses.activate
def test_wait_for_completion_posts_the_pending_request_and_returns_body(sample_request):
    responses.add(
        responses.POST,
        f"{BASE_URL}/generate",
        json={"status": "success", "output_path": sample_request["output_path"], "message": "ok"},
        status=200,
    )
    client = make_client()
    job_id = client.submit(sample_request)
    result = client.wait_for_completion(job_id, timeout=30.0, poll_interval=1.0)

    assert result["status"] == "success"
    assert result["output_path"] == sample_request["output_path"]

    import json as jsonlib

    sent = jsonlib.loads(responses.calls[0].request.body)
    assert sent == sample_request
...
def test_wait_for_completion_raises_when_job_never_submitted():
    ...
    with pytest.raises(SubmitError, match="no pending request"):
        client.wait_for_completion("never_submitted.wav", timeout=30.0, poll_interval=1.0)


@responses.activate
def test_wait_for_completion_raises_submit_error_on_validation_rejection(sample_request):
    responses.add(
    ...
    with pytest.raises(SubmitError, match="422"):
        client.wait_for_completion(job_id, timeout=30.0, poll_interval=1.0)


@responses.activate
def test_wait_for_completion_raises_execution_error_on_500(sample_request):
    responses.add(
        responses.POST,
        f"{BASE_URL}/generate",
...
@responses.activate
def test_wait_for_completion_raises_execution_error_on_non_success_status(sample_request):
    responses.add(
        responses.POST,
        f"{BASE_URL}/generate",
...
@responses.activate
def test_wait_for_completion_wraps_connection_failure(sample_request):
    responses.add(
...
@responses.activate
def test_wait_for_completion_raises_poll_timeout_on_request_timeout(sample_request):
...
def test_fetch_output_downloads_bytes_by_basename():
    ...
    client = make_client()
    data = client.fetch_output({"output_path": "F:/ACE-Step/outputs/out_7.wav"})
...
@responses.activate
def test_generate_end_to_end(sample_request):
    responses.add(
        responses.POST,
        f"{BASE_URL}/generate",
        json={"status": "success", "output_path": sample_request["output_path"], "message": "ok"},
    ...
    responses.add(
        responses.GET,
        f"{BASE_URL}/output/{sample_request['output_path']}",
        body=b"WAVDATA",
        status=200,
    )
    client = make_client()
    data = client.generate(sample_request, timeout=30.0, poll_interval=1.0)
