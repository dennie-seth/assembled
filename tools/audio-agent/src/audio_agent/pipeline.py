"""Ties the whole T-0082 arrow together: license gate -> generate (under the
GPU lock) -> save -> descend seam -> provenance (13-asset-pipeline.md §1,
§4.2). `generate()` is what the `audio` agent and the CLI (`cli.py`) call.
Mirrors `comfy_client.pipeline.generate`.
...
from audio_agent.provenance import ProvenanceRecord, build_provenance_record
