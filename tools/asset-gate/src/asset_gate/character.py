...
def sweep_character_gate(
    ...
    *root* must pass CHR-1 presence, the idle-class frame-delta cap, a
    ...
    `determine_character_motion_fidelity` and `check_motion_score_binding` --
    the one function the `character-gate` CLI subcommand exposes, and the one
    both `ci-asset-gate.yml` and the board's reviewer route invoke, so there
    is exactly one enforcement path instead of two that can silently drift
